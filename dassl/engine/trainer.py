...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self):
        ...
        for self.batch_idx, batch in enumerate(self.train_loader_x):
            ...
            for name, meter in losses.meters.items():
                try:
                    if hasattr(meter.avg, 'dim') and meter.avg.dim() > 0:
                        print(
                            f"[DEBUG] ERROR: Meter '{name}' is a TENSOR with shape {meter.avg.shape}, value: {meter.avg}. This will cause the crash.")
                        scalar_value = meter.avg.mean().item()
                        self.write_scalar("train/" + name, scalar_value, n_iter)
                    else:
                        self.write_scalar("train/" + name, meter.avg, n_iter)
                except Exception as e:
                    print(
                        f"[DEBUG] Exception for meter '{name}': Type={type(meter.avg)}, Value={meter.avg}, Exception={e}")
                    raise e
            self.write_scalar("train/lr", self.get_current_lr(), n_iter)
